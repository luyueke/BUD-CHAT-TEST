...
public static class XCodePostProcessBuild
{
	...
	public static void OnPostProcessBuild(BuildTarget target, string pathToBuiltProject)
	{
        ...
        // Keep Unity-generated framework search paths, then append project-specific paths.
        pbxProject.AddBuildProperty(frameworkTarget, "FRAMEWORK_SEARCH_PATHS", "$(PROJECT_DIR)/Frameworks");
        pbxProject.AddBuildProperty(frameworkTarget, "FRAMEWORK_SEARCH_PATHS", "$(SRCROOT)/Libraries/Plugins/iOS");

        SetNatCorder(frameworkTarget, pbxProject, pathToBuiltProject);
    ...
    static void SetNatCorder(string frameworkTarget, PBXProject pbxProject, string pathToBuiltProject)
    {
        const string natCorderXcodePath = "Frameworks/NatCorder.framework";
        var sourceFrameworkPath = Path.Combine(Application.dataPath, "Plugins/NatCorder/Plugins/iOS/NatCorder.framework");
        var destFrameworkPath = Path.Combine(pathToBuiltProject, natCorderXcodePath);
        if (!Directory.Exists(sourceFrameworkPath))
        {
            Debug.LogWarning($"NatCorder framework source not found: {sourceFrameworkPath}");
            return;
        }
        ...
        CopyDirectory(sourceFrameworkPath, destFrameworkPath);

        var fileGuid = pbxProject.AddFile(natCorderXcodePath, natCorderXcodePath, PBXSourceTree.Source);
