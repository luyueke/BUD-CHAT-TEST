...
- (void)initializeGravityEngineWithClientId:(NSString *) clientId withUserName:(NSString *)userName withVersion:(int)version withAsaEnable:(bool)enableAsa withIdfa:(NSString *) idfa withIdfv:(NSString *)idfv withCaid1:(NSString *)caid1_md5 withCaid2:(NSString *)caid2_md5 withSyncAttribution:(bool)syncAttribution withCreateTime:(long)createTimestamp withCompany:(NSString *)company withChannel:(NSString *)channel withSuccessCallback:(CallbackWithSuccess)successCallback withErrorCallback:(CallbackWithError)errorCallback{
    ...
    network.serverURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@/event_center/api/v1/user/initialize/?access_token=%@&client_id=%@", _config.configureURL, _config.accessToken, encodedClientId]];
...
- (void)wkWebViewGetUserAgent:(void (^)(NSString *))completion {
    __block int done = 0;
    @try {
        // 创建一个定时器，如果在指定时间内没有完成，则调用completion
        NSTimer *timer = [NSTimer scheduledTimerWithTimeInterval:5.0 // 设置超时时间，例如5秒
                                                          target:self
        ...
        [self.wkWebView evaluateJavaScript:@"navigator.userAgent" completionHandler:^(id __nullable userAgent, NSError * __nullable error) {
            // 检查是否有错误，如果有错误则调用completion并传递nil
            if (error) {
                completion(nil);
            } else {
                // 调用completion并传递userAgent
                completion(userAgent);
            }
            done = 1;
        ...
        GELogError(@"userAgent exception %@", exception.description);
        if (done == 0) {
            completion(nil);
        }
    } @finally {
        GELogDebug(@"user agent done");
    }
...
- (void)onTimeout:(NSTimer *)timer {
    GELogInfo(@"222222");
    // 从userInfo中获取completion
    void (^completion)(NSString *) = timer.userInfo;
    
    // 调用completion并传递nil，表示超时
    completion(nil);
