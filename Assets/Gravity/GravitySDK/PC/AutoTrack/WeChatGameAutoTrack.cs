...
#elif GRAVITY_BYTEDANCE_TT_GAME_MODE
            TT.GetAppLifeCycle().OnShow += objects =>
            {
                GravitySDKLogger.Print("bytedance game on show");
                var query = objects["query"] ?? new Dictionary<string, object>();
                var scene = objects["scene"];
                GravityEngineAPI.TrackMPShow((Dictionary<string, string>) query, "" + scene,
                    GetAutoTrackProperties(AUTO_TRACK_EVENTS.MP_SHOW.ToString()));
            };
#endif
        }
...
//             {
//                 action.Invoke(param);
//                 GravityEngineAPI.TrackMPShare("APP_MESSAGE", GetAutoTrackProperties(AUTO_TRACK_EVENTS.MP_SHARE.ToString()));
//             });
//             WX.OnShareTimeline(action =>
//             {
//                 GravityEngineAPI.TrackMPShare("TIME_LINE", GetAutoTrackProperties(AUTO_TRACK_EVENTS.MP_SHARE.ToString()));
//             });
// #endif
//         }
    }
