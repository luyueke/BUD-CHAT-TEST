...
public class IncubationCabinLinkBox : MonoBehaviour
{
    ...
    private Texture2D _scanTex;     // scanRawImg 预览：显示 crop[1] 对比度增强后的图
    ...
    private IEnumerator RequestCameraAndStart()
    {
        ...
        // 预览纹理（crop[1] 对比度增强后的画面）
        if (_scanTex != null) Destroy(_scanTex);
        _scanTex = new Texture2D(_cropW[1], _cropH[1], TextureFormat.RGBA32, false);
        if (scanRawImg != null)
        {
    ...
    private void StopCamera()
    {
        ...
        if (scanRawImg != null) { scanRawImg.texture = null; scanRawImg.gameObject.SetActive(false); }
        if (_scanTex != null) { Destroy(_scanTex); _scanTex = null; }
        _cropBufs = null;
    }
    ...
    private void TickScan()
    {
        ...
        // 一次 GetPixels32，填充 3 个裁剪变体，然后对比度增强（主线程，纯数组运算，很快）
        Color32[] allPixels = _camTexture.GetPixels32();
        BuildCrops(allPixels, camW);
        for (int i = 0; i < 3; i++)
            EnhanceContrast(_cropBufs[i], _cropW[i] * _cropH[i]);

        // 用增强后的 crop[1] 刷新 scanRawImg，方便直接观察 ZXing 实际收到的图像
        if (scanRawImg != null && _scanTex != null && scanRawImg.gameObject.activeSelf)
        {
            _scanTex.SetPixels32(_cropBufs[1]);
    ...
    // 直方图拉伸：把裁剪区的亮度范围拉伸到 [0,255]，同时转灰度
    // 两次遍历：第一次找 min/max，第二次线性拉伸写回
    // 整数运算，对 ~105K 像素约 0.5ms，主线程可接受
    private static void EnhanceContrast(Color32[] buf, int n)
    {
        int lo = 255, hi = 0;
        for (int i = 0; i < n; i++)
        {
            // BT.601 整数近似：Y ≈ (r*77 + g*150 + b*29) >> 8
            int y = (buf[i].r * 77 + buf[i].g * 150 + buf[i].b * 29) >> 8;
            if (y < lo) lo = y;
            if (y > hi) hi = y;
        }

        int range = hi - lo;
        if (range < 8) return;  // 图像几乎全灰，跳过拉伸避免噪声放大

        for (int i = 0; i < n; i++)
        {
            int y = (buf[i].r * 77 + buf[i].g * 150 + buf[i].b * 29) >> 8;
            int stretched = (y - lo) * 255 / range;
            if (stretched < 0) stretched = 0;
            else if (stretched > 255) stretched = 255;
            byte g = (byte)stretched;
            buf[i] = new Color32(g, g, g, 255);
        }
    }
