...
namespace RTG
{
    ...
    public class RTGizmosEngine : MonoSingleton<RTGizmosEngine>, IHoverableSceneEntityContainer
    {
        ...
        private List<GizmoHandleHoverData> _hoverDataCollection = new List<GizmoHandleHoverData>(10);
        private YesNoAnswer _yesNoAnswerCache = new YesNoAnswer();
        private List<RTSceneGizmoCamera> _sceneGizmoCamerasCache = new List<RTSceneGizmoCamera>();
        private List<Gizmo> _sortedGizmosCache = new List<Gizmo>();
        ...
        public bool IsSceneGizmoCamera(Camera camera)
        {
            ...
            // BUD:GC优化
            _sceneGizmoCamerasCache ??= new List<RTSceneGizmoCamera>();
            _sceneGizmoCamerasCache.Clear();
            foreach (var item in _sceneGizmoCameras)
            {
                if (item.Camera == camera)
                {
                    _sceneGizmoCamerasCache.Add(item);
                }
            }
            return _sceneGizmoCamerasCache.Count != 0;
        }
        ...
        public ObjectTransformGizmo CreateObjectMoveGizmo()
        {
            ...
            transformGizmo.CurGizmo = moveGizmo;
        ...
        public ObjectTransformGizmo CreateObjectRotationGizmo()
        {
            ...
            transformGizmo.CurGizmo = rotationGizmo;
        ...
        public ObjectTransformGizmo CreateObjectScaleGizmo()
        {
            ...
            transformGizmo.CurGizmo = scaleGizmo;
        ...
        {
            // UnityEngine.Profiling.Profiler.BeginSample("RTG Update_SystemCall");
            ...
            if (canUpdateHoverInfo)
            {
                ...
                // BUD:GC优化
                _yesNoAnswerCache ??= new YesNoAnswer();
                _yesNoAnswerCache.Reset();
                if (CanDoHoverUpdate != null) CanDoHoverUpdate(_yesNoAnswerCache);
                if (_yesNoAnswerCache.HasNo) canUpdateHoverInfo = false;
            }
            ...
            // BUD:GC优化
            _hoverDataCollection ??= new List<GizmoHandleHoverData>(10);
            _hoverDataCollection.Clear();
            ...
            foreach (var gizmo in _gizmos)
            {
                    ...
                    isDeviceInsideFocusCamera && deviceHasPointer && focusCameraCanRenderGizmos)
                {
                    ...
                    if (handleHoverData != null) _hoverDataCollection.Add(handleHoverData);
                }
            }

            GizmoHandleHoverData hoverData = null;
            if (canUpdateHoverInfo && _hoverDataCollection.Count != 0)
            {
                SortHandleHoverDataCollection(_hoverDataCollection, inputDevicePos);

                hoverData = _hoverDataCollection[0];
        ...
        public void Render_SystemCall(Camera renderCamera)
        {
            ...
            if (Settings.EnableGizmoSorting)
            {
                ...
                // BUD:GC优化
                // UnityEngine.Profiling.Profiler.BeginSample("add gizmo Range");
                _sortedGizmosCache.Clear();
                _sortedGizmosCache.AddRange(_gizmos);
                // UnityEngine.Profiling.Profiler.EndSample();

                // TODO:会造成128B GC
                _sortedGizmosCache.Sort(delegate(Gizmo g0, Gizmo g1)
                {
                ...
                foreach (var gizmo in _sortedGizmosCache)
                {
