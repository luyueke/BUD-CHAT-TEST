...
namespace RTG
{
    ...
    public class ObjectTransformGizmo : GizmoBehaviour
    {
        public class ObjectRestrictions
        {
            ...
            public Vector3 AdjustScaleVectorSetOne(Vector3 scaleVector)
            {
            ...
            public Vector3 AdjustScaleVectorSetZero(Vector3 scaleVector)
            {
                Vector3 finalScaleVec = scaleVector;
                if (!CanScaleAlongAxis(0)) finalScaleVec[0] = 0.0f;
                if (!CanScaleAlongAxis(1)) finalScaleVec[1] = 0.0f;
                if (!CanScaleAlongAxis(2)) finalScaleVec[2] = 0.0f;
        ...
        private IGizmoBehaviour _curGizmo;
        ...
        public GameObject TargetPivotObject { get { return _targetPivotObject; } }
        //S4-0调整
        public IGizmoBehaviour CurGizmo
        {
            get { return _curGizmo; }
            set { _curGizmo = value; }
        }

        public bool isSpecialScale = false;
        ...
        public override void SetSnapEnabled(bool isEnabled)
        {
            ...
            _curGizmo.SetSnapEnabled(_isSnapEnabled);
        }
        ...
        private void ScaleObject(GameObject gameObject, Vector3 scalePivot)
        {
            ...
            Vector3 scaleOffset = Gizmo.ScaleDragOffset;

            ObjectRestrictions restrictions = GetObjectRestrictions(gameObject);
            if (restrictions != null)
            {
                if (!restrictions.IsAffectedByHandle(Gizmo.DragHandleId)) return;
                scaleFactor = restrictions.AdjustScaleVectorSetOne(scaleFactor);
                scaleOffset = restrictions.AdjustScaleVectorSetZero(scaleOffset);
            }
            ...
            if (ScaleConstraintEnabled)
            {
                ...
                if (_isSnapEnabled)
                    objectTransform.SetWorldScaleEx(scaleOffset);
                else
