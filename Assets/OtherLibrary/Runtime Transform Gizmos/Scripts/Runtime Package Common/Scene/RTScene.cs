...
namespace RTG
{
    ...
    public class RTScene : MonoSingleton<RTScene>
    {
        ...
        public bool IgnoreUIElementHovered {
            get;
            set;
        } = false;
        ...
        {
            return !IgnoreUIElementHovered && GetHoveredUIElements().Count != 0;
        }

        private List<RaycastResult> hoveredUIRaycastResults = new List<RaycastResult>();

        public List<RaycastResult> GetHoveredUIElements()
        {
            // BUD:GC优化
            hoveredUIRaycastResults.Clear();
            // No event system? Return an empty list.
            // if (EventSystem.current == null) return new List<RaycastResult>();
            if (EventSystem.current == null) return hoveredUIRaycastResults;
            ...
            if (!inputDevice.HasPointer()) return hoveredUIRaycastResults;
            ...
            EventSystem.current.RaycastAll(eventDataCurrentPosition, hoveredUIRaycastResults);
            hoveredUIRaycastResults.RemoveAll(item => item.gameObject.GetComponent<RectTransform>() == null);

            return hoveredUIRaycastResults;
        }
