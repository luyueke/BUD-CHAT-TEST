...
namespace ChocDino.UIFX
{
	...
	public class FilterStackTextMeshPro : UIBehaviour
	{
		...
		[SerializeField] float _relativeFontSize = 0f;
		...
		protected override void OnEnable()
		{
			...
			_displayMaterial = new Material(Shader.Find(BlendShaderPath));
			...
			if (_textMeshPro)
			{
				...
				for (int i = 0; i < materialCount; i++)
				{
					...
					if (meshInfo.vertexCount > 0)
					{
						...
						_screenRect.AddVertexBounds(this.transform, meshInfo.vertices, meshInfo.vertexCount);
					}
				}
			}
		...
		private void SetupMaterialTMPro(Material material, Camera camera, RectInt textureRect)
		{
			...
			if (camera == null)
			{
				// For some reason in Overlay mode we have to multply these by 2 to get correct rendering otherwise
		...
		void RenderToTexture()
		{
			...
			if (_composite.Start(canvasCamera, _screenRect.GetTextureRect()))
			{
				...
				if (_filters != null && sourceTexture)
				{
					...
					if (!_rt)
					{
						_rt = RenderTexture.GetTemporary(sourceTexture.width, sourceTexture.height, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
						_rt2 = RenderTexture.GetTemporary(sourceTexture.width, sourceTexture.height, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
						#if UNITY_EDITOR
		...
		protected virtual void Update()
		{
			...
			if (_textMeshPro.canvas)
			{
				if (_textMeshPro.canvas.renderMode == RenderMode.WorldSpace)
				{
