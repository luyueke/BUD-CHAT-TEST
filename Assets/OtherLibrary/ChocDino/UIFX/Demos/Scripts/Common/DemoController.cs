...
namespace ChocDino.UIFX.Demos
{
	...
	{
		[Header("Demos")]
		[SerializeField] GameObject[] _demos = null;
		...
		void ChangeDemo(int demoIndex)
		{
			...
			if (_textProgress) { _textProgress.text = string.Format("{0}/{1}", (_demoIndex + 1), DemoCount); }
			if (_scrollbar)
			{
				float newScrollbarValue = _demoIndex / (float)(DemoCount - 1);
