...
public class TimerManager : InstMonoBehaviour<TimerManager>
{
    // 私有变量
    ...
    public void RunAndStop(BudTimer timer)
    {
        // timer不存在
        if (timer == null)
            return;
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _runningLst.Count; i++)
        {
            ...
            while (time < 0)
            {
                ...
                {
                    LoggerUtils.LogError("TimerManager Callback?.Invoke Error:" + e.ToString() + e.StackTrace);
                }
