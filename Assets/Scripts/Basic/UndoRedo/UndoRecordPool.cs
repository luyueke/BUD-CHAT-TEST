...
namespace Basic.UndoRedo
{
    ...
    /// </summary>
    public class UndoRecordPool: GameInstance<UndoRecordPool>
    {
        // private Deque
        private List<UndoRecord> _undoDeque = new List<UndoRecord>();
        private Stack<UndoRecord> _redoStack = new Stack<UndoRecord>();

        private Action<List<UndoRecord>, int> _removeFromPoolCallBack;
        ...
        private void OnRemoveRecord(List<UndoRecord> records,int opType)
        {
            ...
            _removeFromPoolCallBack?.Invoke(records,opType);
        ...
        public void AddRecordsRemovedListener(Action<List<UndoRecord>, int> callback)
        {
            _removeFromPoolCallBack += callback;
        }

        public void RemoveRecordsRemovedListener(Action<List<UndoRecord>, int> callback)
        {
            _removeFromPoolCallBack -= callback;
        }

        public void ClearRecordsRemovedListener()
        {
            _removeFromPoolCallBack = null;
        }
