...
namespace Com.TheFallenGames.OSA.Util.PullToRefresh
{
	...
	public class PullToRefreshBehaviour : MonoBehaviour, IScrollRectProxy, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
        ...
        float _PullAmountNormalized = .1f;
		...
		/// <summary></summary>
		[SerializeField][Tooltip("是否拉取加载刷新")]
		bool _AllowPullFromEnd = true;
		
		[SerializeField]
		bool _AllowPullFromHead = true;
        ...
        bool _AutoHideRefreshGizmo = false;
...
#pragma warning restore 0649
		...
		[Tooltip("OnRefresh by Slide up")]
		public UnityEvent OnRefreshWithSlideUp = null;
...
#pragma warning restore 0067
		...
		void Awake()
        {
            ...
            _RefreshGizmo = GetComponentInChildren<PullToRefreshGizmo>(true); // self or children
			if (_ScrollRect)
			{
				// May be null
		...
		public void OnBeginDrag(PointerEventData eventData)
		{
			...
			if(curDragSign > 0 &&_AllowPullFromHead)
				return;
        ...
        public void OnDrag(PointerEventData eventData)
		{
			...
			if (IsScrollRectNearToTarget(_CurrentDragSign) && _State != StateEnum.AFTER_RELEASE_WAITING_FOR_GIZMO_TO_HIDE && isDrag)
			{
				return;
			}
        ...
        public void OnEndDrag(PointerEventData eventData)
		{
			...
			if (proceedRefresh)
			{
				...
				if (_AllowPullFromHead  && _CurrentDragSign < 0 &&OnRefreshWithSlideUp != null)
				{
					OnRefreshWithSlideUp.Invoke();
				}
				if (_RefreshGizmo)
		...
		private void Update()
		{
			if (ScrollRectProxy.IsInitialized && IsScrollRectNearToTarget(_CurrentDragSign) && _State != StateEnum.AFTER_RELEASE_WAITING_FOR_GIZMO_TO_HIDE && isDrag)
			{
				isDrag = false;
				ShowGizmoIfNeeded();
				_State = StateEnum.AFTER_RELEASE_WAITING_FOR_GIZMO_TO_HIDE;
		...
		bool IsScrollRectNearToTarget(int targetDragSign)
		{
			double normPos = ScrollRectProxy.GetNormalizedPosition();
			if (ScrollRectProxy.IsHorizontal)
				normPos = 1d - normPos;

			double size = ScrollRectProxy.GetContentSize();

			double nearValue = ScrollRectProxy.GetViewportSize();
			
			// Debug.LogError( normPos+"---"+size+"---"+(normPos * size).ToString()+"--------"+nearValue);
			
			if (targetDragSign == -1 && (normPos * size <= nearValue))
				return true;
