...
public class PlantTaskGiftGroupItem : MonoBehaviour
{
    public int Index;
    ...
    private void Awake()
    {
        ...
        Buy.onClick.AddListener(OnBtnBuy);
        if(PreviewBtn != null)
        {
    ...
    {
        UIManager.Inst.OpenPanel<ProfileThemePreviewPanel>(PanelId.ProfileThemePreviewPanel, ProfileTheme.PlantTree);
    }

    private void OnEnable()
    {
        Debug.Log("PlantTaskGiftGroupItem");
        SetData();
    }

    public void SetData()
    {
        //productId = PlantTreeSystem.Inst.productIds[Index];

        var ls = IAPDataManager.Inst.productRes.treePlantingDayPackageList;

        mdata = ls[Index];// ls?.Find(x => { return x.productId.EndsWith(productId); });
        //Debug.LogError("SetData ls=" + JsonConvert.SerializeObject(mdata));
        if (mdata == null)
        {
            return;
        }
    ...
    void OnBtnBuy()
    {
     //   Debug.LogError("PlantTaskGiftGroupItem 111");
        // 如果已购买或数据为空，则不处理
        if (mdata == null || mdata.isPaid == 1)
        {
            return;
        }
     //   Debug.LogError("PlantTaskGiftGroupItem 222");
    ...
    private void ShowPackReward()
    {
        ...
        if (Icon3 != null)
        {
            ...
            {
                IconSp = Icon3.sprite,
                RewardAmount = int.Parse(Count3.text.Remove(0, 1)),
                //rewardSpecial = Name,
            };
    ...
    {
        UIManager.Inst.OpenPanel<ProfileThemePreviewPanel>(PanelId.ProfileThemePreviewPanel, ProfileTheme.LoverGift);
    }
}