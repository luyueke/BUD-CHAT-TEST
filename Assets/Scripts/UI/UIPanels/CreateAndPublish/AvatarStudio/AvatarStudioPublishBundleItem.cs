...
public class AvatarStudioPublishBundleItem : AvatarStudioBaseItem {
    ...
    public override void Init(Action<DraftListItem> onSelect, Action onCreateSelect, DraftListItem data) {
        ...
        _curData = data;
        SelectedImage.SetActive(data.selected);
        ...
        if (skinInfo != null) {
            ...
            if (priceIcon != null)
            {
                priceIcon.sprite = PgcUtils.LoadCurrencyIcon(materialInfo.paymentInfo.currencyType, gameObject);
            }
            if (isShowIcon) {
                price.SetText(priceValue.ToString());
            } else {
