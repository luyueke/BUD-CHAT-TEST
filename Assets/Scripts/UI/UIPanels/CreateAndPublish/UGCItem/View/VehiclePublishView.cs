...
public class VehiclePublishView : BaseDetailView
{
    ...
    protected override void OnNextBtnClick()
    {
        ...
        vehicleEditData.skinActionDraftInfo.Upload((info, isSuccess) => {
            if (gameObject == null)
            {
                return;
            }
            if (isSuccess)
            {
                vehicleEditData.skinActionDraftInfo.PublishDraftToServer((vehicleInfo, newSuccess) => {
                    if (newSuccess)
                    {
                        base.OnNextBtnClick();
                        MessageHelper.Broadcast<UgcBaseInfo>(MessageName.UgcVehicleDidPublishedNew, vehicleInfo);
                    }
                    else
                    {
            ...
            {
                ((LoadingButton)nextBtn).HideLoading();
                TipPanel.ShowToast($"{vehicleEditData.skinActionDraftInfo.GetUploadMessage()}");
            }
        });
    ...
    {
        if (vehicleEditData.metaDataBytes == null && !string.IsNullOrEmpty(editData.GetInfo().metaDataUrl))
        {
            vehicleEditData.metaDataBytes = Asset.LoadRemoteAssetSync(editData.GetInfo().metaDataUrl);
        }
        if (vehicleEditData.metaDataBytes != null)
        {
            var itemPb = MapPbDataTool.ParsePropPb(vehicleEditData.metaDataBytes);
    ...
    private void OnPriceToggleClick(int value, bool isCustom = false)
    {
        ...
        if (vehicleEditData.GetVehicleInfo().paymentInfo == null)
        {
            vehicleEditData.GetVehicleInfo().paymentInfo = new PaymentInfo()
            {
                price = value,
                currencyType = vehicleEditData.currencyType
            };
        }
        else
        {
            vehicleEditData.GetVehicleInfo().paymentInfo.price = value;
            vehicleEditData.GetVehicleInfo().paymentInfo.currencyType = vehicleEditData.currencyType;
        }
    ...
    protected override void SetUgcShaderStyle(UgcShaderStyle ugcStyle)
    {
       ...
       int ty =  vehicleEditData.GetVehicleInfo()?.vehicleType == 1 ? 6 : 7;
        var priceData = DataTables.GetOtherPublishPrice(ty);
    ...
    protected override void SyncEditData()
    {
        ...
        var paymentInfo = vehicleEditData.GetVehicleInfo().paymentInfo;
        ...
        else
        {
            ...
            if (vehicleEditData.GetVehicleInfo().paymentInfo.currencyType == CurrencyType.PinkCoin)
            {
                priceToggleDic.TryGetValue(vehicleEditData.GetVehicleInfo().paymentInfo.price,
                    out findToggle);
            }
            ...
            if (findToggle == null)
            {
                ...
                priceEditBtn.transform.Find("HasPrizeGroup/Image").GetComponent<Image>().sprite = PgcUtils.LoadCurrencyIcon(vehicleEditData.currencyType, gameObject);
            }
            else
            {
    ...
    {
        var coverUrl = vehicleEditData.skinActionDraftInfo.GetCoverUrl();
        cover.Load(coverUrl);
    }
