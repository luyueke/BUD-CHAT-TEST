...
public class AnniversaryCelePackSubView4Item : MonoBehaviour
{
    ...
    void OnPreviewClicked(int index)
    {
        try
        {
            var info = AnniversaryCelePackMgr.Inst.RewardTypeList[AnniversaryCelePackMgr.TaskId_4][_eventId][index];
            PreviewManager.Inst.ShowPreview(info);
        }
        catch (System.Exception e)
        {
            LoggerUtils.Log("AnniversaryCelePackSubView4Item OnPreviewClicked failed: " + e.Message);
        }
    }
    void OnEnable()
    {
    ...
    {
        AnniversaryCelePackMgr.Inst.ClaimTaskReward(AnniversaryCelePackMgr.TaskId_4, _taskItemData.eventId, null, null);
    }
