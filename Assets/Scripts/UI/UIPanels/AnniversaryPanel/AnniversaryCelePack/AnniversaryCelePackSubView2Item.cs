...
public class AnniversaryCelePackSubView2Item : MonoBehaviour
{
    ...
    void OnPreviewClicked(int index)
    {
        try
        {
            var info = AnniversaryCelePackMgr.Inst.RewardTypeList[AnniversaryCelePackMgr.TaskId_2][_eventId][index];
            PreviewManager.Inst.ShowPreview(info);
        }
        catch (System.Exception e)
        {
            LoggerUtils.Log("AnniversaryCelePackSubView2Item OnPreviewClicked failed: " + e.Message);
        }
    ...
    {
        AnniversaryCelePackMgr.Inst.ClaimTaskReward(AnniversaryCelePackMgr.TaskId_2, _taskItemData.eventId, null, null);
    }
