...
public class AnniversaryCelePackSubView3Item : MonoBehaviour
{
    ...
    void OnPreviewClicked(int index)
    {
        try
        {
            var info = AnniversaryCelePackMgr.Inst.RewardTypeList[AnniversaryCelePackMgr.TaskId_3][_eventId][index];
            PreviewManager.Inst.ShowPreview(info);
        }
        catch (System.Exception e)
        {
            LoggerUtils.Log("AnniversaryCelePackSubView3Item OnPreviewClicked failed: " + e.Message);
        }
    }
    void OnEnable()
    {
    ...
    {
        AnniversaryCelePackMgr.Inst.ClaimTaskReward(AnniversaryCelePackMgr.TaskId_3, _taskItemData.eventId, null, null);
    }
