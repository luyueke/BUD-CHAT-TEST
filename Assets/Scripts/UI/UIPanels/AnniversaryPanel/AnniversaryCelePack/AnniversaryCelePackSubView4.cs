...
/// 豪华补给 礼包
/// </summary>
public class AnniversaryCelePackSubView4 : PackBaseView
{
    ...
    public AnniversaryCelePackSubView4Item[] items;
    public AnniversaryCelePackView parentView;
    string productName = "豪华补给";
    public void Awake()
    {
        InitConfig(PaidPackageType.DeluxeSupplies);
        Btn_buy.onClick.AddListener(OnBuyBtnClick);
        this._paidPackageListItem = AnniversaryCelePackMgr.Inst.PaidPackage2ItemDict[PaidPackageType.DeluxeSupplies];
        if (this._paidPackageListItem == null)
        {
            AnniversaryCelePackMgr.Inst.GetProductInfo(PaidPackageType.DeluxeSupplies, OnServerDataUpdate);
        }
        else
        {
    ...
    protected override void OnBuySuccess(string orderId)
    {
        ...
        AnniversaryCelePackMgr.Inst.GetTaskData(AnniversaryCelePackMgr.TaskId_4);
    }
    ...
    public void RefreshUI()
    {
        ...
        var taskInfo = AnniversaryCelePackMgr.Inst.TaskId2InfoDict[AnniversaryCelePackMgr.TaskId_4];
        if (taskInfo == null)
        {
            return;
        }
