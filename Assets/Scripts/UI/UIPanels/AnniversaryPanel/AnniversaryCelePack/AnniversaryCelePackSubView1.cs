...
/// 曙光惊喜 礼包
/// </summary>
public class AnniversaryCelePackSubView1 : PackBaseView
{
    ...
    public AnniversaryCelePackSubView1Item[] items;
    public AnniversaryCelePackView parentView;
    string productName = "曙光惊喜";

    public void Awake()
    {
        InitConfig(PaidPackageType.DawnSurprise);
        Btn_buy.onClick.AddListener(OnBuyBtnClick);
        this._paidPackageListItem = AnniversaryCelePackMgr.Inst.PaidPackage2ItemDict[PaidPackageType.DawnSurprise];
        if (this._paidPackageListItem == null)
        {
            AnniversaryCelePackMgr.Inst.GetProductInfo(PaidPackageType.DawnSurprise, OnServerDataUpdate);
        }
        else
        {
    ...
    protected override void OnBuySuccess(string orderId)
    {
        ...
        AnniversaryCelePackMgr.Inst.GetTaskData(AnniversaryCelePackMgr.TaskId_1);
    }
    ...
    public void RefreshUI()
    {
        ...
        var taskInfo = AnniversaryCelePackMgr.Inst.TaskId2InfoDict[AnniversaryCelePackMgr.TaskId_1];
        if(taskInfo == null)
        {
            return;
        }
        ...
        {
            items[0].RefreshUI(taskInfo.eventList[0],1);
        }
    }
}
