...
public class GoldenSnakePack : MonoBehaviour
{
    ...
    private List<string> pgcIds = new List<string>() { "40300508", "40300506" };
    private void Start()
    {
        ...
        item1.onClick.AddListener(() =>
        {
            ...
            panel.SetNewyearEventPreview(new List<string>() { pgcIds[0] }, "快乐对对碰", "", "", "新春限定", null);
        });

        item2.onClick.AddListener(() =>
        {
            var panel = UIManager.Inst.OpenPanel<RewardPreviewPanel>(PanelId.RewardPreviewPanel);
            panel.SetNewyearEventPreview(new List<string>() { pgcIds[1] }, "红包枪", "", "", "新春限定", null);
        });
    }
