...
public class S8LimitedTimeComeback : PackBaseView {
    ...
    private void Start() {
        InitConfig(PaidPackageType.S8TwentyYuanPackage);
        EventTracking.LoadEvent.ReportPopupStatus(RechargeId.S8TwentyYuanPackage.ToString());
        // InitBg();
        InitClickListener();
    ...
    protected override void OnTaskListUpdate(TaskListRsp taskListRsp) {
        ...
        for (int i = 0; i < packItemNodeList.Count; i++) {
            packItemNodeList[i].SetData(TaskId, eventList[i], taskItemData => {
                RefreshTaskStatus();
            }, () => {
    ...
    private void CreateItems(List<RewardItem> rewardDataList) {
        ...
        for (var i = 0; i < rewardDataList.Count; i++) {
            if (string.IsNullOrEmpty(rewardDataList[i].rewardName2)) {
                ...
                taskItem._rewardItem.rewardIcon1 += "_rewardIcon";
    ...
    private void OnBuyBtnClick() {
        ...
        if (IAPDataManager.Inst.IsOfficialChannel()) {
            ...
            panel.SetCallback(paymentType => {
                Purchase(paymentType, productInfo, packViewConfig.productName);
            });
            return;
        }
    ...
    private void OnPreviewBtnClick() {
        ...
        paidPackRewardDatas.Add(new PaidPackRewardData() {
            pgcIds = new List<string>() { "40100326" },
            name = "轻盈舞蹈",
            isBundle = false
        });
        var panel = UIManager.Inst.OpenPanel<PaidPackRewardPanel>(PanelId.PaidPackRewardPanel);
        panel.itemBgColorStr = "#F9618D";
        panel.bundleContentBgColorStr = "#F9618D";
