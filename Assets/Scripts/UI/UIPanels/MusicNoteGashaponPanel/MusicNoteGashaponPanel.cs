...
public class MusicNoteGashaponPanel : BaseGashaponView
{
    ...
    protected CurrencyType exchangeCurrency = CurrencyType.MiaoCoin;
    ...
    {
        string typeId = "lottery.musicalPudding.hotAirBalloon";
        return typeId;
    }
    ...
    protected override void OnGashaponInfoUpdate(GashaponInfoRsp infoRsp)
    {
        ...
        {
            tipsText.text = string.Format("3次为1轮，再抽取{0}次必点亮音符", 3 - (infoRsp.luckyProgressInfo?.roundIndex ?? 0));
    ...
    public override void OnGashaOnceRsp(GashaponRsp gashaponRsp)
    {
        ...
        ponyGashaponTurntable.StartRotation(hitPrize, (hitPrize) =>
        {
            ...
            {
                TipPanel.ShowToast(string.Format("已抽取{0}次，返还{1}喵币", gashaponRsp.gachaTimes, gashaponRsp.gachaReturn.amount));
    ...
    bool ContainsBigReward(GashaponRsp gashaponRsp)
    {
        ...
        List<int> pgcIdList = new List<int> { 160200003, 33600001 };
        foreach (var reward in gashaponRsp.rewardList)
        {
            ...
            if (reward.rewardType == (int)BUDRewardType.RewardMusicNoteCrystal && reward.amount >= 5) //5个炫彩水晶
            {
                return true;
            }
        }
    ...
    private void OnPriviewBtnClick()
    {
        ...
        List<GashaponPonyPreviewItemData> gashaponPonyPreviewItemDataList = new List<GashaponPonyPreviewItemData>(){
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_7").RewardList,
                title = "惊喜大奖"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_6").RewardList,
                title = "奖池 (音符x6)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_5").RewardList,
                title = "奖池 (音符x5)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_4").RewardList,
                title = "奖池 (音符x4)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_3").RewardList,
                title = "奖池 (音符x3)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_2").RewardList,
                title = "奖池 (音符x2)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_1").RewardList,
                title = "奖池 (音符x1)"
            },
            new GashaponPonyPreviewItemData() {
                rewardList = GashaponDataManager.Inst.gashaponData("lottery.musicalPudding.hotAirBalloon_0").RewardList,
                title = "基础奖池"
            },
        };
    ...
    private void OnExchangeBtnClick()
    {
        ...
        var viewPath = GashaponUtils.ViewBasePath + "MusicNoteGashapon/preview_bg.png";
        var list = gashaponData.ExchangeList;
        var idx = list.FindIndex(x => x.ExchangeId == 568); //把热气球放在第一位
        if (idx > 0)
        {
            var item = list[idx];
            list.RemoveAt(idx);
        ...
        var exchangePanel = UIManager.Inst.OpenPanel<GashaponExchangePanel>(PanelId.GashaponExchangePanel, new GashaponExchangeParam
        {
            ...
            title = "兑换商店",
            gashaponData = gashaponData,
            rewardCurrency = CurrencyType.MusicNoteCrystalShards,
            rewardCurrency1 = CurrencyType.MusicNoteCrystal,
    ...
    private void OnTwistClick()
    {
        ...
        if (!GashaponUtils.CurrencyIsEnough((int)CurrencyType.MiaoCoin, gashaponInfoRsp.singleDrawDiscountedPrice))
        {
            ...
            UIManager.Inst.OpenPanel(PanelId.CatRechargePanel, val);
    ...
    {
        var rulePath = "Assets/Loadable/UI/UIPanel/MusicNoteGashaponPanel/Rule.json";
