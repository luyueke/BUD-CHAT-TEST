...
// 演员卡商城列表视图：结构与 MusicStoreListView 一致。
public class ActorCardStoreListView : MonoBehaviour
{
    public ActorCardStoreListAdapter adapter;
    private ActorCardStoreListDataLoader requester = new ActorCardStoreListDataLoader();
    ...
    public void GetFirstPageDatas(Action<List<RecommendItemData>> complete = null)
    {
        requester.GetSectionInfoData(resultAction: datas =>
        {
            ...
            var list = datas ?? new List<RecommendItemData>();
            // 列表首位固定为「去创作」入口占位项
            list.Insert(0, ActorCardStoreItemView.CreateDesignEntryData());
            adapter.Data.ResetItems(list);
