...
public class DarkShadowFeatherView : BaseGashaponView
{
    ...
    public List<DarkShadowFeatherItem> leftItems;
    ...
    void InitItems()
    {
        ...
        tipsText.text = "恭喜！你已集齐暗影冥羽奖池所有商品！";
        ...
        foreach (var K in leftItems) {
            ...
            if (!ishave)
            {
                ...
                tipsText.text = "每次抽取都不会重复奖励，9次之内必解锁暗影瞑羽";
            }
        }
    }
    ...
    private void OnTwistClick()
    {
        ...
        if (!GashaponUtils.CurrencyIsEnough((int)data.CurrencyType, gashaponInfoRsp.singleDrawPrice))
        {
            ...
            GashaponDataManager.Inst.ShowCurrencyNoEnough((int)data.CurrencyType, val);
            return;
        }
