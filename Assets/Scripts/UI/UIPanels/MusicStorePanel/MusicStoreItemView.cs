...
// 乐谱商城列表 item：从 FittingRoom 的 Ugc+乐谱 流程迁出，结构参照 TimbreStoreItemView，
// 数据由 ToneInfo 改为 MusicScoreInfo，展示价格/已拥有/音数(15音·22音)。
public class MusicStoreItemView : MonoBehaviour
{
    ...
    {
        var fixedData = _curData?.UgcInfo as MusicScoreInfo;
        if (fixedData == null)
        {
            return;
        }
        ...
        // 已拥有判断三路并行（参照 PoseStoreItemView）：自己创作 ∥ 服务端 consumed ∥ 本地背包已有。
        // 只看 consumed 会漏掉「已在背包但 sectionInfoV2 未标记 consumed」的乐谱，导致已拥有仍显示可购买。
        ...
        else
        {
            ...
            if (price > 0)
            {
                ...
                assetName.text = Mathf.CeilToInt(price).ToString();
            }
            else
            {
