...
// 姿势商城列表适配器：结构与 ActorCardStoreListAdapter 一致。
public class PoseStoreListAdapter : GridAdapter<GridParams, PoseStoreListItemHolder>
{
    ...
    protected override void UpdateCellViewsHolder(PoseStoreListItemHolder newOrRecycled)
    {
        RecommendItemData data = Data[newOrRecycled.ItemIndex];
        newOrRecycled.UpdateViews(data, curSelectId, OnSelectItem);

        // 「去创作」占位项：不加载封面
        if (PoseStoreItemView.IsDesignEntry(data))
        {
...
public class PoseStoreListItemHolder : CellViewsHolder
{
    public RemoteImageBehaviour Rm_Cover;
    public PoseStoreItemView draftsItem;

    public override void CollectViews()
    {
        base.CollectViews();
        Rm_Cover = GameObjectEx.FindChildByName(views, "RemoteIcon").GetComponent<RemoteImageBehaviour>();
        draftsItem = root.GetComponentInParent<PoseStoreItemView>();
    }
