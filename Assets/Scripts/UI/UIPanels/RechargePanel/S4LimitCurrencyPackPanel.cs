...
public class S4LimitCurrencyPackPanel : BasePanel<S4LimitCurrencyPackPanel>
{
    ...
    private string _taskId = "LimitedTimeCurrencyPack";
    ...
    public override void OnCreate()
    {
        ...
        if (itemObj)
        {
            ...
            item?.InitCustomBgItem("#9C5AFF", atlasPath,
                new List<string>()
                {
    ...
    private void TrackEvent(BillingResultResponse billingResultResponse)
    {
        ...
        if (isUploadData)
        {
            ...
            trackData.Add("method", "S4LimitCurrencyPack");
    ...
    private void GenerateContent()
    {
        var ugcAsset =
            Loader.Load<TextAsset>(
                "Assets/Loadable/UI/RechargePanel/config/S4LimitCurrencyPackConfig.json", this.gameObject);
            ...
            IAPDataManager.Inst.GetProductInfo(res =>
            {
                ...
                if (paidPackageList != null && this != null)
                {
                    PaidPackageListItem packageListItem =
                        paidPackageList.Find(x => x.packageType == (int)PaidPackageType.LimitedTimeCurrencyPack);
                    if (packageListItem == null)
                    {
                        return;
                    }
        ...
        IAPDataManager.Inst.GetTaskList(_taskId, (b, taskInfoResponse) =>
        {
            ...
            for (int i = 0; i < startPackItems.Count; i++)
            {
                startPackItems[i].SetData(_taskId, eventList[i], taskItemData => { RefreshTaskStatus(); }, () =>
                {
                    ...
                    {
                        TipPanel.ShowToast("购买限时货币礼包获取对应礼品哦");
                    }
                }, PaidPackageType.LimitedTimeCurrencyPack);
            }
        });
    }
