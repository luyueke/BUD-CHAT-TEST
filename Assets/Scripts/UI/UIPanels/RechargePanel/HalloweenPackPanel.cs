...
public class HalloweenPackPanel : BasePanel<HalloweenPackPanel>
{
    ...
    private string _taskId = "HalloweenPack";
    ...
    public override void OnCreate()
    {
        ...
        if (itemObj)
        {
            ...
            item?.InitCustomBgItem("#641cb4", atlasPath,
                new List<string>()
                {
                    "halloween_bg1", "halloween_bg2", "halloween_bg3"
                });
    ...
    private void TrackEvent(BillingResultResponse billingResultResponse)
    {
        ...
        if (isUploadData)
        {
            ...
            trackData.Add("method", "HalloweenPackPanel");
    ...
    private void GenerateContent()
    {
        var ugcAsset =
            Loader.Load<TextAsset>(
                "Assets/Loadable/UI/RechargePanel/config/HalloweenPackConfig.json", this.gameObject);
            ...
            IAPDataManager.Inst.GetProductInfo(res =>
            {
                ...
                if (paidPackageList != null && this != null)
                {
                    PaidPackageListItem packageListItem =
                        paidPackageList.Find(x => x.packageType == (int)PaidPackageType.HalloweenPack);
                    if (packageListItem == null)
                    {
                        return;
                    }
    ...
    private void LoadDefaultUI(PaidPackageListItem packageListItem)
    {
        ...
        endTime.gameObject.SetActive(!string.IsNullOrEmpty(packageListItem.endDate));
        endTime.text = "距售卖时间结束还有: " + packageListItem.endDate;
    }
    ...
    private void RefreshTaskStatus()
    {
        IAPDataManager.Inst.GetTaskList(_taskId, (b, taskInfoResponse) =>
        {
            ...
            taskEndTime.text = "距任务结束还有: " + _taskInfoData.endDate;
            for (int i = 0; i < startPackItems.Count; i++)
            {
                startPackItems[i].SetData(_taskId, eventList[i], taskItemData => { RefreshTaskStatus(); }, () =>
                {
                    ...
                    {
                        TipPanel.ShowToast("购买万圣限定礼包获取对应礼品哦");
                    }
                }, PaidPackageType.HalloweenPack);
            }
        });
    }
