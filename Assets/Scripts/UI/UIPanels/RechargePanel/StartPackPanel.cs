...
public class StartPackPanel : BasePanel<StartPackPanel>
{
    ...
    [SerializeField] private Text priceTips1;
    [SerializeField] private Text rewardTips1;
    [SerializeField] private Text rewardTips2;
    ...
    private string _taskId = "RainyRibbitPack";
    ...
    public override void OnCreate()
    {
        ...
        if (itemObj)
        {
            ...
            item?.InitCustomBgItem("#cae67f", atlasPath,
                new List<string>()
                {
                    "startpack_bg1", "startpack_bg2", "startpack_bg3", "startpack_bg4"
                });
    ...
    {
        var productInfo = IAPDataManager.Inst.GetPriceInfo(ProductIdType.product_budpremiumvaluepack);
        priceText.SetLocalText(productInfo.priceLocal);
        priceTips1.SetLocalText("{0}即可立刻获得",productInfo.priceLocal);
        rewardTips1.SetText("360");
        rewardTips2.SetText("40");
    }
    ...
    private void TrackEvent(BillingResultResponse billingResultResponse)
    {
        ...
        if (isUploadData)
        {
            ...
            trackData.Add("method", "StartPackPanel");
    ...
    {
        string configPath = "Assets/Loadable/UI/RechargePanel/config/StartPackConfig.json";
#if PACKAGE_TYPE_US
        configPath = "Assets/Loadable/UI/RechargePanel/config/StartPackConfig_US.json";
#endif
        var ugcAsset = Loader.Load<TextAsset>(configPath, this.gameObject);
        ...
        panel.ShowRewards(new List<CommonRewardItemData>()
        {
            new CommonRewardItemData()
            {
                IconSp = XAssetLoaderMgr.Inst.LoadSpriteInAltas(spriteatlasPath, "oneyuanpack_reward",
                    gameObject),
                RewardAmount = 1,
                rewardName = LocalizationManager.Inst.GetLocalizedText("6元礼包")
            }
        });
    }
            ...
            IAPDataManager.Inst.GetProductInfo(res =>
            {
                ...
                if (paidPackageList != null && this != null)
                {
                    PaidPackageListItem packageListItem =
                        paidPackageList.Find(x => x.packageType == (int)PaidPackageType.RainyRibbitPack);
                    if (packageListItem == null)
                    {
                        return;
                    }
    ...
    private void LoadDefaultUI(PaidPackageListItem packageListItem)
    {
        ...
        taskEndObj.gameObject.SetActive(packageListItem.isPaid == 1);
        ...
        IAPDataManager.Inst.GetTaskList(_taskId, (b, taskInfoResponse) =>
        {
            ...
            for (int i = 0; i < startPackItems.Count; i++)
            {
                startPackItems[i].SetData(_taskId, eventList[i], taskItemData => { RefreshTaskStatus(); }, () =>
                {
                    ...
                    {
                        TipPanel.ShowToast("购买6元礼包获取对应礼品哦");
                    }
                }, PaidPackageType.StartPack);
            }
        });
    }
    ...
    private void InitClickListener()
    {
        ...
        previewBtn.onClick.AddListener(() =>
        {
            ...
            {
                bgColor = "#cae67f",
                rewardItemBgColor = "#9fc67d",
                atlasPath = spriteatlasPath,
                iconList = new List<string>()
                {
                    "startpack_bg1", "startpack_bg2", "startpack_bg3"
                },
                rewardList = pgcIds
            };
    ...
    private void BuyOnePack(ConfirmPaymentPanel.PaymentType paymentType)
    {
        ...
        _iapTrackData.item = LocalizationManager.Inst.GetLocalizedText("6元礼包");
