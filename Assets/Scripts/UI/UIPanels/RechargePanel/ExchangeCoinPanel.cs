...
public class ExchangeCoinPanel : BasePanel<ExchangeCoinPanel>
{
    ...
    public static void OpenPenlByPinkCoin(float need)
    {
        if(JudgePinkCoin(need))
        {
    ...
    public static bool JudgePinkCoin(float need)
    {
        ...
        Debug.Log($"JudgePinkCoin gem={gemNum},pink={pinkNum},need={need}");
        if (gemNum < need)
        {
            UIManager.Inst.OpenPanel<GetMorePinkCoinPanel>(PanelId.GetMorePinkCoinPanel, need);
            return false;
        }
        return true;
    }
                ...
                panel.ShowRewards(new List<TaskRewardData>()
                {
                    new TaskRewardData()
                    {
                        ...
                        rewardType = _exchangeType switch
                        {
                            ...
                            CurrencyType.ShrimpYuan=>(int)BUDRewardType.RewardBabyShrimp,
