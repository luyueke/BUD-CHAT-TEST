...
namespace GameUI
{
    public class GEParkWorkDetail : MonoBehaviour
    {
        ...
        {
            var info = GameStudioUtils.GetBaseInfo(this._draftListItem);
            Txt_MapName.text = info.name;
            Txt_LastEditTime.SetLocalText("最后编辑 {0}",TimestampConverter.ConvertToDateTimeString(this._draftListItem.mapInfo.updateTime));
            Remote_MapCover.Load(info.cover);
        }
        ...
        private void OnBtnPublishClick()
        {
            ...
            if (!AIParkUgcEditPanel.CheckUgcHospitalDataIsLegal(_draftListItem.mapInfo))
            {
                return;
            }
