...
{
    public class WearingCard : BaseCard
    {
        [SerializeField] private WearingItem ItemPrefab;
        [SerializeField] private Transform ContentRoot;

        private WearingItem CreateItem()
        {
            WearingItem newItem = Instantiate(ItemPrefab, ContentRoot);
        ...
        public bool RefreshWithData(string uid,List<WearingInfo> resList)
        {
            ...
            foreach (var itemData in resList)
            {
                ...
                {
                    var config = Es.DataTables.GetAvatarCommonData(itemData.pgcId);
                    if (config == null)
                    {
                        continue;
                    }
