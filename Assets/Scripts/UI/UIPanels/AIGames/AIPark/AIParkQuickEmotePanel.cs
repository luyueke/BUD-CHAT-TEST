...
public class AIParkQuickEmotePanel : BasePanel<AIParkQuickEmotePanel>
{
    public Button BgBtn;
    ...
    public AIParkQuickMsgView _quickMsgView;

    public CButton _quickMsgViewStateBtn;

    public AIParkInputAnimation _moveAnimation;
    ...
    public override void OnShow(params object[] args)
    {
        ...
        InitUI();
        _quickMsgView.InitQuickMsgScrollView(this);
        _quickMsgView.gameObject.SetActive(false);
    ...
    private void InitUI()
    {
        ...
        _quickMsgViewStateBtn.gameObject.SetActive(false); //暂时不要
    ...
    public override void OnHidden()
    {
        ...
        {
            var curNpcBev = AIPark_CharacterManager.Inst.GetNpc(_currentInteractNpcID);
            if (curNpcBev != null)
            {
    ...
    private void OnKeyboard(string msg)
    {
        ...
        _aIParkGuestPanel.SendInput(msg);
    ...
    {
        LoggerUtils.Log("AIParkQuickEmotePanel OnBgClick");
        GameAINpcChatManager_Park.Inst.ResetChatTargetBehaviour();
        if (!AIParkGuideMgr.Inst.IsDuringGuideFirstInGame())
        {
            ...
            if (!string.IsNullOrEmpty(npcId))
            {
                ...
                if(npc.IsFollowPath() || (npc.GetCurrentState() is IdleState) || (npc.GetCurrentState() is TalkWithPlayerState)){
                    AIPark_CharacterManager.Inst.RevertChatNpcToLastChapter(npcId);
                }
            }

        }
        AIParkGuideMgr.Inst.CancelChatGuide();
        CloseSelf();
    }
    ...
    public void OnQuickEmoteBtnClick(int index)
    {
        ...
        MessageHelper.Broadcast(MessageName.OnS11ChatEmoteClick, emoteID, _quickEmoteList[index].desc);
    ...
    {
        var npcBehaviour = AIPark_CharacterManager.Inst.GetNpc(_currentInteractNpcID);
        if (npcBehaviour)
        {
            ...
            else
            {
                ...
                {
                    npcBehaviour.GetChapterHandler()?.ForceChangeState(ActionType.Idle);
                }
                var talkState2 = npcBehaviour.StartTalkWithPlayer();
                talkState2.StartPlayNpcEmote(emoteID);
