...
public class AIHospitalHUDPanel : MonoBehaviour
{
    public AIHospitalNpcHudItem HudItemPrefab;
    ...
    // 缓存已创建的HUD项
    private List<AIHospitalNpcHudItem> hudItems = new List<AIHospitalNpcHudItem>();
    // 当前活跃的HUD项数量
    ...
    private AIHospitalNpcHudItem CreateHudItem()
    {
        // 创建HUD项实例
        AIHospitalNpcHudItem item = Instantiate(HudItemPrefab, HudItemParent);
    ...
    {
        // 获取视野内的医院NPC
        List<AIHospital_CharacterBehaviour> visibleCharacters = GetAllVisibleHospitalCharacters(colliders);
        
        // 更新并显示HUD项，返回已使用的HUD项引用
        List<AIHospitalNpcHudItem> usedItems = UpdateHudItems(visibleCharacters);
    ...
    private List<AIHospitalNpcHudItem> UpdateHudItems(List<AIHospital_CharacterBehaviour> characters)
    {
        List<AIHospitalNpcHudItem> usedItems = new List<AIHospitalNpcHudItem>();
        Dictionary<AIHospital_CharacterBehaviour, AIHospitalNpcHudItem> existingPairs = new Dictionary<AIHospital_CharacterBehaviour, AIHospitalNpcHudItem>();
        ...
        foreach (var item in hudItems)
        {
            ...
            {
                AIHospital_CharacterBehaviour npc = item.GetCurrentNpc();
        ...
        foreach (var character in characters)
        {
            ...
            // 获取或创建新的HUD项
            AIHospitalNpcHudItem hudItem = GetAvailableHudItem();
    ...
    private AIHospitalNpcHudItem GetAvailableHudItem()
    {
        // 尝试查找一个未激活的HUD项
        foreach (var item in hudItems)
        {
    ...
    private void UpdateHudItemPosition(AIHospitalNpcHudItem hudItem, Vector3 worldPosition)
    {
    ...
    /// <summary>
    /// 获取视野范围内所有的AIHospital_CharacterBehaviour
    /// </summary>
    private List<AIHospital_CharacterBehaviour> GetAllVisibleHospitalCharacters(Collider[] colliders)
    {
        List<AIHospital_CharacterBehaviour> visibleCharacters = new List<AIHospital_CharacterBehaviour>();
        ...
        {
            // 直接查找AIHospital_CharacterBehaviour组件
            AIHospital_CharacterBehaviour[] characters = collider.GetComponentsInParent<AIHospital_CharacterBehaviour>();
