...
public class AIHospitalQuickMsgView : MonoBehaviour
{
    private AIHospitalQuickEmotePanel _quickEmotePanel;
    ...
    public void InitQuickMsgScrollView(AIHospitalQuickEmotePanel quickEmotePanel,bool inGuide = false)
    {
    ...
    private void OnQuickMsgClick(int index)
    {
        if (index >= 0 && index < _quickMsgList.Count)
        {
            ...
            // 发送消息到聊天面板
            var guestPanel = UIManager.Inst.FindPanel<AIHospitalGuestPanel>(PanelId.AIHospitalGuestPanel);
            if (guestPanel != null)
            {
