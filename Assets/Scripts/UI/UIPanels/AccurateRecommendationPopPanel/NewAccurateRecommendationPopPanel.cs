...
public class NewBieAvataInfo
{
    public int id;
    public string avataJason;
}
public class NewBieAvataConfig
{
    public List<NewBieAvataInfo> configs;
}

public class NewAccurateRecommendationPopPanel : BasePanel<NewAccurateRecommendationPopPanel>
{   
    // UI组件
        ...
        if (labelContainer != null)
        {
            ...
            if (labelPrefab != null)
            {
                foreach (var label in labels)
                {
                    ...
                    // 获取标签组件
                    var labelItem = labelObj.GetComponent<NewBieLabelItem>();
                    if (labelItem != null)
                    {
    ...
    // 处理标签点击事件
    private void OnLabelClicked(NewBieLabelItem labelItem)
    {
    ...
    private void UpdateConfirmButtonState()
    {
        ...
        if (confirmButton != null)
        {
            ...
            confirmButton.gameObject.SetActive(key);
        }
    }
    ...
    void OnSkiClick()
    {
        AccountDataManager.Inst.RefreshUserInfoSignin();
    ...
    // 关闭面板
    private void CloseSelf(bool skip = false)
    {
        if (_isClosed) return;
