...
namespace Mopsicus.Plugins {
    ...
    public class MobileInputField : MobileInputReceiver {
        ...
        /// <summary>
        /// inputfield布局跟layoutgroup布局会冲突,这里加个重置inputobjectText方法，给外部调用
        /// </summary>
        public void ResetInputObjectText(Text text)
        {
            _inputObjectText?.gameObject.SetActive(false); //隐藏原来的text
            _inputObjectText = text;
        }
	...
	/// <summary>
        ...
        public void SetFocus (bool isFocus) {
            Debug.Log("SET_FOCUS _isMobileInputCreated  " + _isMobileInputCreated);
#if (UNITY_IOS || UNITY_ANDROID) && !UNITY_EDITOR
            if (!_isMobileInputCreated) {
