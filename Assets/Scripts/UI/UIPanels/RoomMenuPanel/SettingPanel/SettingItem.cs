...
using Basic.Utils;
    ...
    public abstract class SettingItem : MonoBehaviour
    {
        ...
        {
            title.SetLocalText(settingItemData.title);
            if (settingItemData.textLength > 0 && settingItemData.widthLimit > 0 &&
                title.preferredWidth > settingItemData.widthLimit)
            {
            ...
            {
                UIManager.Inst.OpenPanel<SettingTipsPanel>(PanelId.SettingTipsPanel, settingItemData.tips, title.transform);
            });
        }
    }
