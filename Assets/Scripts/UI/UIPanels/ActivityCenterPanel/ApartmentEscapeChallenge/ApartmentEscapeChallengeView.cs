...
public class ApartmentEscapeChallengeView : ActivityBaseView
{
    [SerializeField] private ApartmentEscapeChallengeDailyView handDailyView;

    [SerializeField] private ApartmentEscapeChallengeRewardItem rewardItemPrefab;
    ...
    private Dictionary<int, ApartmentEscapeChallengeRewardItem> rewardItems =
        new Dictionary<int, ApartmentEscapeChallengeRewardItem>();
    ...
    private List<string> pgcIds = new List<string>(){ "10400352", "11300255"};
    ...
    public override void Init(ActivityInfo info)
    {
        ...
        foreach (var rewardEventId in rewardEventIds)
        {
            ...
            var rewardData = new CommonRewardItemData()
            {
                ...
                rewardSpecial = eventInfo.rewardSpecial
            };
    ...
    private void ClaimAllReward()
    {
        ...
        NetworkManager.Inst.SendHttpRequest(HttpUrlDefine.ClaimActivityReward,
            ...
            (content) =>
            {
                ...
                isSending = false;
                OnClaimAllSuccess(avtivityEventClaimResponse, BUDRewardType.RewardKey);
            },
    ...
    private void ShowPgcPreview()
    {
        ...
        panel.SetEventPreview(pgcIds,"优优妹制服套装", "apartment_reward_icon_big",spriteatlasPath, "游玩公寓逃脱模拟器，免费领专属套装","#FFAE64", bgImage.texture);
    }
