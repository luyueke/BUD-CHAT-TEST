...
public class TreasureHuntGiftPanel : BasePanel<TreasureHuntGiftPanel>
{
    [SerializeField] private CButton CloseBtn;
    [SerializeField] private CButton BuyBtn0;
    [SerializeField] private Text BuyText0;
    ...
    {
        base.OnCreate();
        _buyTexts = new[] { BuyText0, BuyText1, BuyText2 };
    ...
    public static bool IsAllPacksPurchased()
    {
        ...
        {
            var pkg = list.Find(x => x.productId == id);
            if (pkg == null || pkg.isPaid == 0) return false;
        }
        return true;
    }
    ...
    private void OnBuyClick(int index)
    {
        ...
        //{
        //    productId = pkg.productId,//  productId,
        //price = pkg.price,//Prices[index],
        //    name = pkg.productName,"寻宝礼包"
        //};
        //Debug.LogError("Pkg info =" + JsonConvert.SerializeObject(pkg));
        gemInfo.price = Prices[index];
        //gemInfo.gemNum = pkg.;
        gemInfo.productId = productId;// pkg.productId;
        gemInfo.name = pkg.productName;
        gemInfo.desc = pkg.productDesc;
    ...
    private void ShowReward(ProductGemInfo productInfo)
    {
        ...
        PgcUtils.CurrencyIconPath.TryGetValue(CurrencyType.Shovel, out iconName);
