...
public class ZongziSummerCarnival : ActivityBaseView
{
    [SerializeField] private ZongziSummerCarnivalDailyView handDailyView;

    [SerializeField] private ZongziSummerCarnivalDailyRewardItem rewardItemPrefab;
    ...
    private Dictionary<int, ZongziSummerCarnivalDailyRewardItem> rewardItems =
        new Dictionary<int, ZongziSummerCarnivalDailyRewardItem>();
    ...
    private List<string> pgcIds = new List<string>(){ "10400473" };
    ...
    public override void Init(ActivityInfo info)
    {
        ...
        MessageHelper.AddListener(MessageName.UpdateHallTask, GetActivityInfoData);
    }
    public void GetActivityInfoData()
    {
        ActivityCenterInfoReq req = new ActivityCenterInfoReq() { idList = new List<string> { "FriesFun" } };
        NetworkManager.Inst.SendHttpRequest(HttpUrlDefine.ActivityList, HttpMethod.POST, JsonConvert.SerializeObject(req), (content) =>
        {
            ...
            var _exchangeActivityInfo = activityResponse.list[0];
            UpdateEventStatus(_exchangeActivityInfo);
        },
        (error) => { });
    }
    public void UpdateEventStatus(ActivityInfo info)
    {   

        if (info == null || info.eventList == null) return;

        handDailyView.UPdateStatus(info.eventList);
    ...
    private void ClaimAllReward()
    {
        ...
        NetworkManager.Inst.SendHttpRequest(HttpUrlDefine.ClaimActivityReward,
            ...
            (content) =>
            {
                ...
                isSending = false;
                OnClaimAllSuccess(avtivityEventClaimResponse, BUDRewardType.RewardTypeFries);
            },
    ...
    private void ShowPgcPreview()
    {
        ...
        panel.SetEventPreview(pgcIds, "萌熊薯条连衣裙", "","", "参与疯狂薯条记，免费领取萌熊薯条连衣裙", "#FF9B9B",bgImage.texture);
    }
    ...
    private void OnClaimAllSuccess(ActivityEventClaimResponse response, BUDRewardType budRewardType)
    {
        ...
        if (response != null)
        {
            foreach (var reward in response.rewardList) { 
                if(reward.rewardType == (int)BUDRewardType.RewardPgcResource) //永久服饰
                {
                    CommonRewardItemData data = new CommonRewardItemData {
                        rewardName = "萌熊薯条连衣裙",
                        rewardType = 5,
                        RewardAmount = reward.amount,
                        pgcId = reward.pgcIdList[0],
                    };
                    rewardList.Add(data);
                }
                else if (reward.rewardType == (int)BUDRewardType.RewardPgcResourceFreeTrail)//临时服饰
                {
                    CommonRewardItemData data = new CommonRewardItemData
                    {
                        rewardName = "萌熊薯条连衣裙试用3天",
                        rewardType = 5,
                        RewardAmount = reward.amount,
                        pgcId = reward.pgcIdList[0],
                    };
                ...
                else //货币
                {
                    CommonRewardItemData data = new CommonRewardItemData();
                    data.IconSp =
                        PgcUtils.LoadRewardIcon((BUDRewardType)reward.rewardType, panel.gameObject);
                    data.rewardName = PgcUtils.GetRewardName((BUDRewardType)reward.rewardType);
                    data.RewardAmount = reward.amount;
                    rewardList.Add(data);
                }
            }
        }
    ...
    {
        bool hasUnlockedEvent = datas.Any(data => data.eventStatus == (int)ClaimStatus.Unlocked);
    ...
    {
        MessageHelper.RemoveListener(MessageName.UpdateHallTask, GetActivityInfoData);
    }
}