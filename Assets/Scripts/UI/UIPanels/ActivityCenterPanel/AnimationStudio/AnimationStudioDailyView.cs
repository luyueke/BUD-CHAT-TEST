...
public class AnimationStudioDailyView : MonoBehaviour {



    [SerializeField] private Transform content;
    [SerializeField] private AnimationStudioItemView itemView;
    ...
    public void InitListUI(List<ActivityEventInfo> datas, string id)
    {
        ...
        {
            var obj = Instantiate(itemView, content);
    ...
    {
        var tmpItemView = eventViews.Find(x => x.EventId == eventId);
        if (tmpItemView == null)
        {
            return;
        }
    ...
    private void OnClaimSuccess(ActivityEventClaimResponse response)
    {
        ...
        var tmpItemView = eventViews.Find(x => x.EventId == response.eventInfo.eventId);
        ...
        ClaimSuccessAction?.Invoke(AnimationEventGroup.Daily, response);
    }
}
