...
public class MusicEventCumulativeView : MonoBehaviour
{
    ...
    [SerializeField] private MusicEventDailyItemView mainItemView;
    
    private List<MusicEventDailyItemView> eventViews = new List<MusicEventDailyItemView>();
    
    private List<string> tabNames = new List<string>() {  "乐器创作", "乐谱创作", "音色创作", "乐器演奏" };
    ...
    public void InitListUI(Dictionary<NotesJumpGroupType, List<ActivityEventInfo>> groupDatas, string activityId)
    {
    ...
    public void RefreshListUI(Dictionary<NotesJumpGroupType, List<ActivityEventInfo>> groupDatas)
    {
        if (groupDatas == null || groupDatas.Count == 0)
        {
            return;
        ...
        var list = groupDatas[(NotesJumpGroupType)index];
        RefreshUI(list);
    }
    ...
    private void RefreshRedDot()
    {
        ...
        for (int i = 0; i < tabNames.Count; i++)
        {
            var list = groupDatas[(NotesJumpGroupType)(i+1)];
    ...
    private void OnClaimSuccess(ActivityEventClaimResponse response)
    {
        ...
        var index = tabNames.FindIndex(x => x == activeCategoryKey) + 1;
        groupDatas[(NotesJumpGroupType)index].Find(x => x.eventId == response.eventInfo.eventId).eventStatus = response.eventInfo.eventStatus;
        RefreshRedDot();
        ClaimSuccessAction?.Invoke((NotesJumpGroupType)index, response);
    }
}
