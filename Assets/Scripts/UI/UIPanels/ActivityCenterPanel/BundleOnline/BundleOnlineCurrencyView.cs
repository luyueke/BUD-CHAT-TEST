...
public class BundleOnlineCurrencyView : MonoBehaviour
{
    ...
    private void OnShowPreview()
    {
       ...
       panel.UpdateUI(iconSprite, "套装活动币", currencyNum.text, "S2赛季套装活动币，活动结束后自动失效，可通过完成活动任务领取，也可通过钻石兑换。");
    }
    
    public void OnExchange()
    {
        ExchangeCoinPanel exchangeCoinPanel = UIManager.Inst.OpenPanel<ExchangeCoinPanel>(PanelId.ExchangeCoinPanel);
        exchangeCoinPanel.SetPianoCoinEventUI(iconSprite, "BundleOnline", i =>
        {
            if (this == null)
            {
                return;
