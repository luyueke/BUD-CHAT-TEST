...
namespace Game
{
    ...
    public class GizmoController : IUndoRecord
    {
        ...
        {
            get { return _isSnapEnabled; }
        }

        private bool _isRotSnapEnabled = false;
        public bool IsRotSnapEnabled
        {
            get { return _isRotSnapEnabled; }
        }

        private bool _isMoveSnapEnabled = false;
        public bool IsMoveSnapEnabled
        {
            get { return _isMoveSnapEnabled; }
        }

        private bool _isScaleSnapEnabled = false;
        public bool IsScaleSnapEnabled
        {
            get { return _isScaleSnapEnabled; }
        }

        public static float CurRate { get; set; } = 1;
        ...
        public bool IsCustomLimitMoveSize {
            get;
            set;
        }
        ...
        public Vector3 CustomMinSize = Vector3.one * -500;
        public Vector3 CustomMaxSize = Vector3.one * 500;
        ...
        public void SetMoveCtr()
        {
            ...
            currentGizmo = gizmo;
            SetMoveSnapEnabled(_isMoveSnapEnabled);
        ...
        public void SetRotateCtr()
        {
            ...
            currentGizmo = gizmo;
            SetRotSnapEnabled(_isRotSnapEnabled);
        }
        ...
        public void SetScaleCtr(Action onCreate, bool norScale = false)
        {
            ...
            gizmo.SetTargetObject(current);
            currentGizmo = gizmo;
            SetScaleSnapEnabled(_isScaleSnapEnabled);
        }
        ...
        public ObjectTransformGizmo CreateMoveGizmo()
        {
            ...
            if (IsCustomLimitMoveSize)
            {
                UseCustomMinMaxMove(moveGizmo.Gizmo);
            }
            ...
            if (fieldInfo != null)
            {
                ...
                {
                    sGizmo.LookAndFeel3D.SetScale(4.5f * CurRate);
                }
            }
        ...
        public void SetSnapEnabled(bool isEnabled)
        {
            ...
            currentGizmo?.SetSnapEnabled(_isSnapEnabled);
        }

        public void SetRotSnapEnabled(bool isEnabled)
        {
            _isRotSnapEnabled = isEnabled;
            rotationGizmo?.SetSnapEnabled(isEnabled);
        }

        public void SetMoveSnapEnabled(bool isEnabled)
        {
            _isMoveSnapEnabled = isEnabled;
            moveGizmo?.SetSnapEnabled(isEnabled);
        }

        public void SetScaleSnapEnabled(bool isEnabled)
        {
            _isScaleSnapEnabled = isEnabled;
            scaleGizmo?.SetSnapEnabled(isEnabled);
        }
        ...
        {
            gizmo.PostDragUpdate -= PoseCustomDragUpdate;
            gizmo.PostDragUpdate += PoseDragUpdate;
        }

        private void PoseDragUpdate(Gizmo giz, int handle)
        {
            if (!current) return;
            if (!IsLimitMoveSize) {
                return;
        ...
        void UseCustomMinMaxMove(Gizmo gizmo)
        {
            gizmo.PostDragUpdate -= PoseDragUpdate;
            gizmo.PostDragUpdate += PoseCustomDragUpdate;
        }
        
        private void PoseCustomDragUpdate(Gizmo giz, int handle)
        {
            if (!current) return;
            if (!IsCustomLimitMoveSize) {
                return;
            ...
            Vector3 ret = current.transform.localPosition;
            ret.x = Mathf.Clamp(ret.x, CustomMinSize.x, CustomMaxSize.x);
            ret.y = Mathf.Clamp(ret.y, CustomMinSize.y , CustomMaxSize.y);
            ret.z = Mathf.Clamp(ret.z, CustomMinSize.z, CustomMaxSize.z);
            current.transform.localPosition = ret;
        }
