...
public class NestedScrollRect : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler {
    ...
    MonoBehaviour scrollRect; //子框
    bool isUpAndDown = true; //子框的滑动方向
    
    public bool useOSA = false;
    public bool osaIsVertical = false;
    void Awake () {
        if (useOSA)
        {
            scrollRect = GetComponent<IOSA>().AsMonoBehaviour;
            isUpAndDown = osaIsVertical;
        }
        else
        {
            ScrollRect srect = GetComponent<ScrollRect>();
            scrollRect = srect;
            isUpAndDown = srect.vertical;
        }
        
        if (parentScrollRect == null && transform.parent) {
            parentScrollRect = transform.parent.GetComponentInParent<ScrollRect>(); //查找父节点的Scrollview
        }
    }
