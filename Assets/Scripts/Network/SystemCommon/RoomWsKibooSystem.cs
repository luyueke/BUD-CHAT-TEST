...
namespace Game
{
    public class RoomWsKibooSystem
    {
        private static RoomWsKibooSystem _instance;
        public static RoomWsKibooSystem Inst
        {
            get
            {
                if (_instance == null)
                {
                    _instance = new RoomWsKibooSystem();
                }
                return _instance;
            }
        }
        ...
        public Action<bool> onWebSocketConnectStateChange;
        ...
        public void SendChatMessageServer(string message)
        {
            if(!IsConnected){
            ...
            SendMessage2Server(JsonConvert.SerializeObject(jsonObject));
        }
        ...
        void ConnectSuccess()
        {
            ...
            onWebSocketConnectStateChange?.Invoke(true);
        }

        void OnMessageRecv(WebSocket ws, string message)
        {
            Debug.Log("Roomwskiboo system OnMessageRecv: " + message);
            try
            {
                RoomChatWsMessage chatWsMessage = JsonConvert.DeserializeObject<RoomChatWsMessage>(message);
                if (chatWsMessage.type == (int)RoomChatWsType.AIEvent)
                {
                    string data = chatWsMessage.data;
                    try
                    {
                ...
                else if (chatWsMessage.type == (int)RoomChatWsType.ChatTopic)
                {
                    string data = chatWsMessage.data;
                    ...
                    if (data == "$$")
                    {
                        MessageHelper.Broadcast(MessageName.NewAddChatMessage, chatStringBuilder.ToString(),false);  //这里都是ai的
                        chatStringBuilder.Clear();
                    }
                    else
                    {
                        chatStringBuilder.Append(data);
                    }
                    Debug.Log("Roomwskiboo system roomChatTopicData:" + data);
                }
