...
using Game.COSXML.Log;
using Game.COSXML.Model.Bucket;
    ...
    public sealed class COSXMLUploadTask : COSXMLTask, IOnMultipartUploadStateListener
    {
        ...
        private void UpdateProgress(long complete, long total, bool isCompleted)
        {
            ...
            if (complete < total)
            {
                ...
                TotalBytes = total;
                Progress = (float)((double)complete / total);
                if (progressCallback != null)
                {
            ...
            else
            {
                ...
                {
                    CompleteBytes = total - 1;
                    TotalBytes = total;
                    Progress = (float)((double)(CompleteBytes - 1) / TotalBytes);
                    if (progressCallback != null)
                    {
