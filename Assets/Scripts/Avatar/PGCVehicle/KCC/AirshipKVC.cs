...
public class AirshipKVC : PGCVehicleBaseKVC
{
    ...
    public override void OnInit(KinematicCharacterMotor motor, bool isSelf, bool isReconstruction = false)
    {
        ...
        bannerAni = GameObjectEx.FindComponentByName<Animator>(motor.gameObject, "vehicle_549_01");
        textMesh = GameObjectEx.FindComponentByName<SuperTextMesh>(motor.gameObject, "Text");
        
        MessageHelper.AddListener<bool>(MessageName.OnPlayerVehicleShow, ToggleBanner);
    }
    ...
    private float _lastToggleTime = -2f;
    private const float BannerToggleCooldown = 2f;
    void ToggleBanner(bool isPress)
    {
        if (Time.time - _lastToggleTime < BannerToggleCooldown)
            return;
        _lastToggleTime = Time.time;
        if (isPress)
        {
            CoroutineManager.Inst.StartCoroutine(ShowText(true));
            bannerAni.Play("ani_hengfu2");
            MessageHelper.Broadcast(MessageName.GameSound, "Vehicle_Skill", "Airship_Skill01", IsSelf ? "Play_Skill_1P" : "Play_Skill_3P", Motor.gameObject);
        }
        else
        {
            CoroutineManager.Inst.StartCoroutine(ShowText(false));
            bannerAni.Play("ani_hengfu");
            MessageHelper.Broadcast(MessageName.GameSound, "Vehicle_Skill", "Airship_Skill02", IsSelf ? "Play_Skill_1P" : "Play_Skill_3P", Motor.gameObject);
        }
    }

    IEnumerator ShowText(bool bo) {
        if (bo)
        {
            float t = 0f;
            while (t < 1f)
    ...
    {
        MessageHelper.RemoveListener<bool>(MessageName.OnPlayerVehicleShow, ToggleBanner);
        base.Release();
    ...
    protected override void OnStateChange(PGCVehicleAniState state)
    {
        ...
        {
            MessageHelper.Broadcast(MessageName.GameSound, "Vehicle_Show", "Airship_Show", IsSelf ? "Play_Show_1P" : "Play_Show_3P", Motor.gameObject);
        }
    }
    ...
    public override void UpdateVehicleVelocity(ref Vector3 currentVelocity, float deltaTime)
    {
        ...
        //     var Seat_1 = GameObjectEx.FindComponentByName<Transform>(Motor.gameObject, "Seat_1");
        //     if (Seat_0 != null && Seat_1 != null)
        //     {
        //         Seat_0.localPosition = new Vector3(0,0.05f,0.6f);
        //         // Seat_0.localRotation = Quaternion.Euler(new Vector3(0, 90, 0));
        //         // Seat_1.localRotation = Quaternion.Euler(new Vector3(0, 90, 0));
        //         Seat_1.localPosition = new Vector3(0,0.05f,-0.6f);
        //     }
        // }
    ...
    public override void OnSkill(int skillId, bool isPress, string extraJson = null)
    {
        ...
        else if(skillId == (int)SkillType.Skill5)
        {
            ToggleBanner(isPress);
            return;
        }
        ...
        MessageHelper.Broadcast(MessageName.GameSound, "Vehicle_Drive", "Airship_Run", IsSelf ? "Play_Drive_1P" : "Play_Drive_3P", Motor.gameObject);
    ...
    {
        strParam = string.IsNullOrEmpty(strParam) ? "Best Wishes!" : strParam;
        textMesh.text = strParam;
    }
}