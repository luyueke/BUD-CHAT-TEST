...
/// 竹蜻蜓 KCC
/// </summary>
public class HopterKCC : BaseKCC {
    ...
    public HopterKCC(string uid, bool isSelf) : base(uid, isSelf) {
    ...
    private Animator GetPGCAnimator() {
        if (pgcAnimator == null) {
            var hatRoot = PlayerAnimCtrl.Wrap.GetBandNode((int)BodyNode.SpecialHatNode);
            pgcAnimator = hatRoot.GetComponentInChildren<Animator>();
        }
