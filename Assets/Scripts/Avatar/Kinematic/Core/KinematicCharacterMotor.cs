...
#pragma warning restore 0649
        ...
        public bool IsOnSimulate => isOnSimulate;
        private bool isOnSimulate = true;
        ...
        public enum UGCVehicleStatus{
            None = 0,//没有乘坐或驾驶载具
            Drive = 1,//驾驶载具
            TakeCar = 2,//乘坐载具
        }

        public UGCVehicleStatus CurUGCVehicleStatus = UGCVehicleStatus.None;

        public bool IsDriveVehicle = false;

        /// <summary>
        /// 是否处于载具状态（召唤/乘坐/驾驶任一场景）。
        /// 用于屏蔽走路脚步音效等“人走路”相关逻辑。
        /// </summary>
        public bool IsVehicleSummoned => IsDriveVehicle || CurUGCVehicleStatus != UGCVehicleStatus.None;
        ...
        private GameObject _lastAttachedGameObject;
        ...
        private Vector3 _vehicleHeight = Vector3.zero;
...
#pragma warning restore 0414 
        ...
        {
            //GetComponentInChildren<CustomBodyTypeController>()
        ...
        private void ResolveInitialOverlaps()
        {
            ...
            while (iterationsMade < MaxDecollisionIterations && !overlapSolved)
            {
                int nbOverlaps = CharacterCollisionsOverlap(_transientPosition, _transientRotation,
                    _internalProbedColliders, CollisionOffset);
        ...
        private float _baseCapsuleHeight = -1f;

        public void SetCapsuleHeightData(BodyType bodyType)
        {
            if (_baseCapsuleHeight < 0)
            {
                _baseCapsuleHeight = CapsuleHeight;
            }

            float targetHeight = _baseCapsuleHeight;
            switch (bodyType)
            {
                case BodyType.Type1:
                case BodyType.Type2:
            ...
            if (_solveMovementCollisions)
            {
                ...
                // 第一步：解穿透（把角色从地下/墙里拔出来）
                ResolveInitialOverlaps();
                
                // // 第二步：强制地面探测和吸附（确保角色站在地面上）
                // if (_solveGrounding)
                // {
                //     // 先向上移动一个安全距离，确保角色在地面之上，然后再向下探测
                //     // 这样可以处理角色陷入很深的情况
                //     float safeUpwardDistance = CapsuleHeight * 0.5f + CapsuleRadius;
                //     _transientPosition += _characterUp * safeUpwardDistance;
                    
                //     // 使用足够大的探测距离，确保能找到地面
                //     CharacterGroundingReport groundingReport = new CharacterGroundingReport();
                //     float probingDistance = CapsuleHeight + CapsuleRadius + GroundDetectionExtraDistance;
                    
                //     // 执行地面探测和吸附
                //     ProbeGround(ref _transientPosition, _transientRotation, probingDistance, ref groundingReport);
                    
                //     // // 如果找到了稳定地面，确保角色正确吸附
                //     // if (groundingReport.IsStableOnGround && !groundingReport.SnappingPrevented)
                //     // {
                //     //     // ProbeGround 已经修改了 _transientPosition，这里不需要额外操作
                //     // }
                // }
            ...
            //this.SetFrameCallBack(1, Time.deltaTime);
            UpdatePhase1(Time.deltaTime);
            UpdatePhase2(Time.deltaTime);
        }

        public void SetVehicleHeight(float height,bool isDrive)
        {
            _vehicleHeight = new Vector3(0, height,0);
        }

        /// <summary>
        /// 计算模型（胶囊底部点）与地面的距离。
        /// 复用 Motor 每帧 ProbeGround 的结果 GroundingStatus.GroundPoint。
        /// 返回：
        /// - >= 0：距离（米）
        /// - < 0：当前未探测到地面（无效）
        /// </summary>
        public float GetModelDistanceToGroundWithoutRaycast()
        {
            if (!GroundingStatus.FoundAnyGround || GroundingStatus.GroundPoint == Vector3.zero)
            {
                return -1f;
            }

            Vector3 capsuleBottomWs = _transientPosition + (_transientRotation * _characterTransformToCapsuleBottom);
            float dist = Vector3.Dot(capsuleBottomWs - GroundingStatus.GroundPoint, _characterUp);
        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            if(IsDriveVehicle)
            {
                float distToGround = GetModelDistanceToGroundWithoutRaycast();
                float diff = distToGround - _vehicleHeight.y;
                if(diff < 0.5f && diff > -0.5f)
                {
                    ...
                    if(distToGround <= 0)
                    {
                        ...
                        // 每帧只修正一次，避免重复调用导致抖动/漂移
                        // diff>0: 当前离地太高 -> 下移；diff<0: 当前离地太低 -> 上移
                        _transientPosition = new Vector3(_transientPosition.x, _transientPosition.y - diff, _transientPosition.z);
                    }
                }
            }
            ...
            if (_solveGrounding)
            {
                ...
                else
                {
                    ...
                    // 驾驶载具时：扩大地面探测范围，用于刷新 GroundingStatus.GroundPoint（例如载具离地较高的情况）
                    // 同时避免 ProbeGround 的 snapping 修改角色位置：只用临时 position 做探测，不回写到 _transientPosition
                    if (IsDriveVehicle)
                    {
                        selectedGroundProbingDistance = Mathf.Max(selectedGroundProbingDistance, CapsuleHeight + GroundDetectionExtraDistance + 10f);
                        var tmpPos = _transientPosition;
                        ProbeGround(ref tmpPos, _transientRotation, selectedGroundProbingDistance, ref GroundingStatus);
                    }
                    else
                    {
            ...
            if (InteractiveRigidbodyHandling)
            {
                ...
                {
                    #region Interactive Collider Handling

                    _lastAttachedGameObject = _attachedGameObject;
                    // if (AttachedRigidbodyOverride)
                    // {
                    //     _attachedRigidbody = AttachedRigidbodyOverride;
                    // }
                    ...
                    {
                        var  interactiveGameObject = GetInteractiveAttachGameObject(GroundingStatus.GroundCollider);
                        if (interactiveGameObject)
                        {
                            _attachedGameObject = interactiveGameObject;
                        }
                        else
                        {
                    ...
                    {
                        GetVelocityFromRigidbodyMovement(_attachedGameObject, _transientPosition, deltaTime,
                            out tmpVelocityFromCurrentAttachedRigidbody,
                            out tmpAngularVelocityFromCurrentAttachedRigidbody);
                    }

                    // Conserve momentum when de-stabilized from an attached rigidbody
                    if (PreserveAttachedRigidbodyMomentum && _lastAttachedGameObject != null &&
                        _attachedGameObject != _lastAttachedGameObject)
                    {
                        BaseVelocity += _attachedRigidbodyVelocity;
                        BaseVelocity -= tmpVelocityFromCurrentAttachedRigidbody;
                    }
                    ...
                    if (GroundingStatus.GroundCollider &&
                        _attachedGameObject != null &&
                        _lastAttachedGameObject == null)
                    {
                        var  interactiveGameObject = GetInteractiveAttachGameObject(GroundingStatus.GroundCollider);
                        if (interactiveGameObject == _attachedGameObject)
                        {
                            BaseVelocity -= Vector3.ProjectOnPlane(_attachedRigidbodyVelocity, _characterUp);
                        }
                    }
                    ...
                    {
                        _isMovingFromAttachedRigidbody = true;
                        var pos = _transientPosition + _attachedRigidbodyVelocity * deltaTime;
                        if (_solveMovementCollisions)
                        {
                            // Perform the move from rgdbdy velocity
                            InternalCharacterMove(ref _attachedRigidbodyVelocity, deltaTime);
                            if (pos != _transientPosition)
                            {
                                Debug.Log($"pos ={pos} _transientPosition={_transientPosition}");
                            }
                        }
                        else
                        {
        ...
        public void UpdatePhase2(float deltaTime)
        {
                    ...
                    else
                    {
                        if (_attachedGameObject)
                        {
                            ...
                            if (CharacterGroundSweep(
                                    ...
                                    out closestHit))
                            {
                                var attachGo = GetInteractiveAttachGameObject(closestHit.collider);
                                if (attachGo == _attachedGameObject &&
                                    IsStableOnNormal(closestHit.normal))
                                {
                                    float distanceMovedUp = (upwardsOffset - closestHit.distance);
                                    _transientPosition = _transientPosition + (_characterUp * distanceMovedUp) +
                        ...
                        if (RigidbodyMode)
                        {
                            ...
                            while (iterationsMade < MaxDecollisionIterations && !overlapSolved)
                            {
                                ...
                                if (nbOverlaps > 0)
                                {
                                    ...
                                    {
                                        // Process overlap
                                        Transform overlappedTransform =
                                            _internalProbedColliders[i].GetComponent<Transform>();
                                        if (Physics.ComputePenetration(
                                                Capsule,
                                                _transientPosition,
                                                _transientRotation,
                                ...
                                if (nbOverlaps > 0)
                                {
                                    for (int i = 0; i < nbOverlaps; i++)
                                    {
                                                ...
                                                out resolutionDistance))
                                        {
                                            ...
                                            {
                                                var probedRigidbody =
                                                    GetInteractiveAttachGameObject(_internalProbedColliders[i]);
                                                if (probedRigidbody != null)
                                                {
                                                    HitStabilityReport tmpReport = new HitStabilityReport();
                                                    tmpReport.IsStable = IsStableOnNormal(resolutionDirection);
            ...
            if(IsDriveVehicle)
            {
                CharacterController.UpdateVehicleVelocity(ref BaseVelocity, deltaTime);
            }
            else
            {
            ...
            CharacterController.CorrectPosition();
        ...
        private bool CheckIfColliderValidForCollisions(Collider coll)
        {
            ...
            {
                if (!InternalIsColliderValidForCollisionsNoRigidbody(coll))
                {
                    return false;
                }
            }
        ...
        private bool InternalIsColliderValidForCollisionsNoRigidbody(Collider coll)
        {
            var go = GetInteractiveAttachGameObject(coll);
            if (go)
            {
                // If movement is made from AttachedRigidbody, ignore the AttachedRigidbody
                if (_isMovingFromAttachedRigidbody && go == _attachedGameObject)
                {
                    return false;
                }
            }
        ...
        /// </summary>
        public void GetVelocityFromRigidbodyMovement(GameObject interactiveGameObject, Vector3 atPoint, float deltaTime, out Vector3 linearVelocity, out Vector3 angularVelocity)
        {
            if (deltaTime > 0f)
            {
                linearVelocity = Vector3.zero;
                angularVelocity = Vector3.zero;
                PhysicsMover physicsMover = interactiveGameObject.GetComponent<PhysicsMover>();
                if (physicsMover)
                {
                ...
                {
                    //TODO:默认centerOfMass值未Zero
                    Vector3 centerOfRotation = interactiveGameObject.transform.TransformPoint(Vector3.zero);
        ...
        /// <summary>
        /// Determines if a collider has an attached interactive GameObject
        /// </summary>
        private GameObject GetInteractiveAttachGameObject(Collider onCollider)
        {
            var pm = onCollider.gameObject.GetComponentInParent<PhysicsMover>(true);
            if (pm != null)
            {
        ...
        public void SetIsOnSimulate(bool isOnSimulate)
        {
            this.isOnSimulate = isOnSimulate;
        }
    }
}