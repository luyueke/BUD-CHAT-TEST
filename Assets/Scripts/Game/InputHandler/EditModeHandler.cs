...
public class EditModeHandler : InputHandler
{
    ...
    private Action<Touch> clickScreenCallback;
    ...
    public float FollowOffsetZ => camTransposer.m_FollowOffset.z;
    ...
    public void AddClickScreenCallback(Action<Touch> callback)
    {
        clickScreenCallback += callback;
    }

    public void RemoveClickScreenCallback(Action<Touch> callback)
    {
        clickScreenCallback -= callback;
    ...
    {
        clickScreenCallback?.Invoke(touch);
    }
    ...
    private void Rotate(float angleVertical, float angleHorizontal)
    {
        if (camFollow == null)
        {
            LoggerUtils.LogError("EditModeHandler Rotate camFollow == null");
            return;
        }
    ...
    private bool mIsDoingRotationSwitch = false;
    public IEnumerator mGenricCamTransformCrtn = null;
    public void OnSceneGizmoHandlePicked(Quaternion targetRotation)
    {
        StopCamTransform();
        GlobalCoroutineUtils.Inst.StartCoroutine(mGenricCamTransformCrtn = DoSmoothRotationSwitch(targetRotation));
    }
    public void StopCamTransform()
    {
        if (mGenricCamTransformCrtn != null)
        {
            GlobalCoroutineUtils.Inst.StopCoroutine(mGenricCamTransformCrtn);
    ...
    {
        if (eCamera == null || camFollow == null)
        {
            LoggerUtils.LogError("EditModeHandler OnPinch eCamera == null || camFollow == null");
            return;
        }
    ...
    void OnMove(Vector2 newCenter)
    {
        if (eCamera == null || camFollow == null)
        {
            LoggerUtils.LogError("EditModeHandler OnMove eCamera == null || camFollow == null");
            return;
        }
