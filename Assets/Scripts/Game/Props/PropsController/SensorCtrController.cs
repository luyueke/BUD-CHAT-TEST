...
namespace Game.Props.PropsController
{
    ...
    /// 道具被开关控制的属性管理器
    /// </summary>
    public class SensorCtrController : BasePropController<SensorCtrController>, IAutoInit
    {
        private const string TAG = "SensorCtrController";
        //被开关控制部分的实体，按照不同类型分类
        ...
        {
            return nodeBehaviour.entity.HasComp<SensorCtrComponent>();
        }
        
        #region 绑定与解除绑定
        private void RemoveControlledEntity(SceneEntity entity)
        {
            if (!entity.HasComp<SensorCtrComponent>()) return;
            GameObjectComponent goCmp = entity.GetComp<GameObjectComponent>();
            
            //从关联的开关中移除
            GlobalNodeManager.Inst.Get<SensorBoxManager>().RemoveControlledId(goCmp.Uid);
        ...
        private Dictionary<uint, SceneEntity> GetSensorCtrDict(GameGlobalEnum.PropControlType controlType)
        {
        ...
        /// <summary>
        /// 根据controlType从SensorCtrComponent获取开关列表
        /// </summary>
        /// <param name="ctrComp"></param>
        /// <param name="controlType"></param>
        /// <returns></returns>
        private List<uint> GetSensorBoxListFromCtrComp(SensorCtrComponent ctrComp,GameGlobalEnum.PropControlType controlType)
        {
        ...
        private void AddControlEntityToManager(SceneEntity entity,GameGlobalEnum.PropControlType controlType)
        {
            ...
            var ctrDict = GetSensorCtrDict(controlType);
        ...
        {
            if(!entity.HasComp<SensorCtrComponent>()) return;
            var ctrComp = entity.GetComp<SensorCtrComponent>();
            uint uid = entity.GetComp<GameObjectComponent>().Uid;
            var ctrDict = GetSensorCtrDict(controlType);
            if (ctrDict != null)
            {
                //当该物体，关于感应盒的某个属性已经为空，才从管理器移除，如Visible跟所有感应盒都无关联，则从管理器的Visible中移除
        ...
        private void CheckRemoveCtrComp(SceneEntity curEntity)
        {
            if (curEntity.HasComp<SensorCtrComponent>())
            {
                ...
                if (totalCount <= 0)
                {
                    curEntity.RemoveComp<SensorCtrComponent>();
                    LoggerUtils.Log("not controll by SensorBox,remove SensorCtrComponent");
                }
            }
        }
        
        private void BindSensorBoxIds(SceneEntity entity, GameGlobalEnum.PropControlType controlType)
        {
            SensorCtrComponent ctrComp = entity.GetComp<SensorCtrComponent>();
            uint ctrId = entity.GetComp<GameObjectComponent>().Uid;
    
            var sensorBoxIds = GetSensorBoxListFromCtrComp(ctrComp, controlType);
            if (sensorBoxIds != null)
            {
                for (int i = 0; i < sensorBoxIds.Count; i++)
                {
                    uint sId = sensorBoxIds[i];
                    GlobalNodeManager.Inst.Get<SensorBoxManager>().AddCtrIdToSensorComp(ctrId, sId, controlType);
                }
            }
        }
        
        /// <summary>
        /// 将SensorCtrComponent节点绑定到对应开关中
        /// </summary>
        /// <param name="nodeBehaviour"></param>
        private void BindToSensorBox(NodeBaseBehaviour nodeBehaviour)
        {
            SceneEntity ctrEntity = nodeBehaviour.entity;
            foreach (var controlType in controlTypes)
            {
                BindSensorBoxIds(ctrEntity, controlType);
            }
        }
      
        
        /// <summary>
        /// 将SensorCtrComponent节点绑定到管理器中
        /// </summary>
        /// <param name="nodeBehaviour"></param>
        private void BindToManager(NodeBaseBehaviour nodeBehaviour)
        {
            SceneEntity ctrEntity = nodeBehaviour.entity;
            SensorCtrComponent ctrComp = ctrEntity.GetComp<SensorCtrComponent>();

            foreach (var controlType in controlTypes)
            {
                var srcUids = GetSensorBoxListFromCtrComp(ctrComp, controlType);
                if (srcUids !=null && srcUids.Count > 0)
                {
        ...
        /// <param name="controlType"></param>
        private void AddSensorBoxIdToCtrComp(SceneEntity entity, uint sId,GameGlobalEnum.PropControlType controlType)
        {
            var ctrComp = entity.GetOrAddComp<SensorCtrComponent>();

            var uidList = GetSensorBoxListFromCtrComp(ctrComp,controlType);
            if (uidList == null)
            {
            ...
            uidList.Add(sId);
            LoggerUtils.Log($"[AddMoveCtrEntity] add to {controlType} Dict ,uid=>{sId}");
        ...
        /// <param name="sId"></param>
        /// <param name="controlType"></param>
        private void RemoveSensorBoxIdFromCtrComp(SceneEntity entity, uint sId,GameGlobalEnum.PropControlType controlType)
        {
            if (!entity.HasComp<SensorCtrComponent>())
            {
                return;
            }
            var ctrComp = entity.GetComp<SensorCtrComponent>();
            var uidList = GetSensorBoxListFromCtrComp(ctrComp,controlType);
            if (uidList !=null && uidList.Contains(sId))
            {
                uidList.Remove(sId);
            }
        }
        ...
        public void BindEntity(SceneEntity entity, uint sId,GameGlobalEnum.PropControlType controlType)
        {
            AddSensorBoxIdToCtrComp(entity,sId,controlType);
            AddControlEntityToManager(entity,controlType);
        }

        public void UnbindEntity(SceneEntity entity, uint sId,GameGlobalEnum.PropControlType controlType)
        {
            RemoveSensorBoxIdFromCtrComp(entity,sId,controlType);
            RemoveControlEntityFromManager(entity,controlType);
            CheckRemoveCtrComp(entity);
        }

        public void UnbindEntityByUid(uint crtId, uint sId,GameGlobalEnum.PropControlType controlType)
        {
            var ctrDict = GetSensorCtrDict(controlType);
            if (ctrDict != null && ctrDict.ContainsKey(crtId))
            {
                var entity = ctrDict[crtId];
                UnbindEntity(entity,sId,controlType);
            }
        }
        ...
        public override void OnCloneNode(NodeBaseBehaviour oldBehaviour, NodeBaseBehaviour newBehaviour)
        {
            ...
            {
                LoggerUtils.Log("SensorCtrController OnCloneNode entity.uid:"+newBehaviour.entity.GetComp<GameObjectComponent>().Uid);
                BindToSensorBox(newBehaviour);
        ...
        {
            base.OnEdit();
            var visibleCtrDict= GetSensorCtrDict(GameGlobalEnum.PropControlType.Visible);
