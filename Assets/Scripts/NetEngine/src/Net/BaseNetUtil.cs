...
namespace NetEngine.src.Net
{
    ...
    {
        private static HashSet<ClientSendServerCmd> _roomCmd;
        ...
        {
            _checkLoginQueue = new Queue<QueueRequest>();
            _roomQueue = new Queue<QueueRequest>();
            _roomCmd = new HashSet<ClientSendServerCmd>
            {
                ClientSendServerCmd.ECmdRoomEnterReq,
                ClientSendServerCmd.ECmdRoomLeaveReq,
            };
            // 创建房间
            // 加入房间
            // 离开房间
        ...
        public void BindSocket(SocketClient socketClient)
        {
            void HandleResponse(DecodeRspResult data) => client.HandleMessage(data);

            void HandleBroadcast(DecodeRspResult data) => NetServer.HandleMessage(data);

            client.BindSocket(socketClient, HandleResponse, HandleBroadcast);
            _server.BindSocket(socketClient, HandleResponse, HandleBroadcast);
        }
        ...
        public string Send(Object body, int subcmd, NetResponseCallback response, Action<ResponseEvent> callback)
        {
            ...
            if (client.SocketClient.Id == (int)ConnectionType.Frame)
            {
                requestCmd = "frame_cmd";
            }
            // if (client.Socket.Id == (int)ConnectionType.KeyFrame)
            // {
            //     requestCmd = "key_frame_cmd";
            // }
            ...
            // 房间操作队列化
            var queue = _roomCmd.Contains((ClientSendServerCmd)subcmd)
                ? _roomQueue
                : _checkLoginQueue;
        ...
        private string SendRequest(QueueRequest queRequest)
        {
            ...
            var seq = client.SendRequest(queRequest.Body, queRequest.Subcmd, queRequest.Response, queRequest.Completed,
                queRequest.RequestCmd, "");
